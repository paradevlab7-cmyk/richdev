...
import { estimateBid, getCollectionDailyStats, getDb, getCollectionRuns, getNotice, getNoticeStats, getSettings, listKeywords, listNotices, listSaved } from "./db";
...
export const appRouter = router({
  ...
  collection: router({ runs: protectedProcedure.query(() => getCollectionRuns()), dailyStats: protectedProcedure.input(z.object({ days: z.number().int().min(1).max(31).default(7) }).optional()).query(({ input }) => getCollectionDailyStats(input?.days ?? 7)), runNow: protectedProcedure.input(z.object({ days: z.number().int().min(1).max(180).default(15) })).mutation(async ({ ctx, input }) => { const result = await collectForUser(ctx.user.id, undefined, 5, input.days); if (result.failures.length === 5) throw new Error(result.failures.map(item => `${item.sourceType}: ${item.message}`).join("\n")); return result; }) }),
