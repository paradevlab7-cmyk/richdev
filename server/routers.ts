...
export const appRouter = router({
  ...
  collection: router({ runs: protectedProcedure.query(() => getCollectionRuns()), runNow: protectedProcedure.input(z.object({ days: z.number().int().min(1).max(180).default(15) })).mutation(async ({ ctx, input }) => { const result = await collectForUser(ctx.user.id, undefined, 5, input.days); if (result.failures.length === 5) throw new Error(result.failures.map(item => `${item.sourceType}: ${item.message}`).join("\n")); return result; }) }),
