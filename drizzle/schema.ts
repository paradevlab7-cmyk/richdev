...
export const collectionRuns = mysqlTable("collection_runs", {
  ...
  totalAvailable: int("totalAvailable").default(0).notNull(),
  storedCount: int("storedCount").default(0).notNull(),
  startPage: int("startPage").default(1).notNull(),
  currentPage: int("currentPage").default(0).notNull(),
  totalPages: int("totalPages").default(0).notNull(),
  queryStartAt: timestamp("queryStartAt"),
  queryEndAt: timestamp("queryEndAt"),
